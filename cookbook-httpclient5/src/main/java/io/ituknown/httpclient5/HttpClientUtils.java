...
public class HttpClientUtils extends SyncClient {
    ...
    public static Headers getStream(String url, CustomRequestConfig config, Consumer<InputStream> streamConsumer) {
        HttpGet request = new HttpGet(url);
    ...
    public static FileEntityResponse downloadUseRemoteName(String url, CustomRequestConfig config, Path dir) {
        HttpGet request = new HttpGet(url);
        return execute(config, request, new FileDownloadUseRemoteNameHttpClientResponseHandler(dir, url));
    }
    ...
    public static Headers postJson(String url, String jsonContent, CustomRequestConfig config, Consumer<InputStream> streamConsumer) {
        HttpPost request = new HttpPost(url);
        request.setEntity(new StringEntity(jsonContent, ContentType.APPLICATION_JSON));
        return execute(config, request, new StreamHttpClientResponseHandler(streamConsumer));
    }
    ...
    public static Headers postJson(String url, byte[] jsonContent, CustomRequestConfig config, Consumer<InputStream> streamConsumer) {
        HttpPost request = new HttpPost(url);
        request.setEntity(new ByteArrayEntity(jsonContent, ContentType.APPLICATION_JSON));
        return execute(config, request, new StreamHttpClientResponseHandler(streamConsumer));
    }
    ...
    public static Headers post(String url, String content, ContentType contentType, CustomRequestConfig config, Consumer<InputStream> streamConsumer) {
        HttpPost request = new HttpPost(url);
        request.setEntity(new StringEntity(content, contentType));
        return execute(config, request, new StreamHttpClientResponseHandler(streamConsumer));
    }
    ...
    public static Headers post(String url, byte[] content, ContentType contentType, CustomRequestConfig config, Consumer<InputStream> streamConsumer) {
        HttpPost request = new HttpPost(url);
        request.setEntity(new ByteArrayEntity(content, contentType));
        return execute(config, request, new StreamHttpClientResponseHandler(streamConsumer));
    }
    ...
    public static <T extends NameValuePair> Headers postForm(String url, final List<T> parameters, CustomRequestConfig config, Consumer<InputStream> streamConsumer) {
        return postForm(url, parameters, config, StandardCharsets.UTF_8, streamConsumer);
    }

    public static <T extends NameValuePair> Headers postForm(String url, final List<T> parameters, CustomRequestConfig config, Charset charset, Consumer<InputStream> streamConsumer) {
        UrlEncodedFormEntity entity = new UrlEncodedFormEntity(parameters, charset);
        HttpPost request = new HttpPost(url);
        request.setEntity(entity);
        return execute(config, request, new StreamHttpClientResponseHandler(streamConsumer));
    ...
    public static Headers postMultipart(String url, MultipartEntityBuilder entity, CustomRequestConfig config, Consumer<InputStream> streamConsumer) {
        HttpPost request = new HttpPost(url);
        request.setEntity(entity.build());
        return execute(config, request, new StreamHttpClientResponseHandler(streamConsumer));
    }
    ...
    public static Headers postFile(String url, File file, ContentType contentType, CustomRequestConfig config, Consumer<InputStream> streamConsumer) {
        HttpPost request = new HttpPost(url);
        request.setEntity(new FileEntity(file, contentType));
        return execute(config, request, new StreamHttpClientResponseHandler(streamConsumer));
    }
